...
export default function Usuarios() {
  ...
  const STATS = [
    ...
    },
    {
      ...
      icon: (
        <svg className={`w-4 h-4 sm:w-6 sm:h-6 ${totalInact > 0 ? 'text-red-500' : 'text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
            d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
        </svg>
      ),
    ...
    <div className="space-y-5">
      ...
      {!cargando && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3">
          {STATS.map(s => (
            <div key={s.label}
              className={`bg-white rounded-2xl border border-gray-200 shadow-sm border-t-4 ${s.accent} p-3 sm:p-4 flex flex-col gap-2 sm:gap-3 hover:shadow-md transition-shadow`}>
              {/* Icono + número en fila */}
              <div className="flex items-start justify-between">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 ${s.iconBg} rounded-xl flex items-center justify-center shrink-0`}>
                  {s.icon}
                </div>
                <span className={`text-2xl sm:text-3xl font-extrabold leading-none ${s.num}`}>{s.value}</span>
              </div>
              {/* Etiqueta + sub */}
              <div className="min-w-0">
                <p className={`text-xs sm:text-sm font-semibold ${s.lbl} truncate`}>{s.label}</p>
                <p className={`text-[10px] sm:text-xs mt-0.5 ${s.sub_text} truncate`}>{s.sub}</p>
              </div>
            </div>
          ))}
        </div>
